...
| W1.6 Repro matrix + note | 🟢 done | `src/echo_frb/repro/matrix/build.py` + `tests/test_matrix.py`. **`docs/reproducibility_matrix.md`** (19 rows: literal EXACT=12/N-A=5/SUBJECTIVE=2; clean-room EXACT=4/APPROX=1/NOT=8/N-A=5/SUBJECTIVE=1), **`docs/WP1_reproduction_note.md`**, **`docs/candidate_reproduction_report.md`** (Appendix-B per candidate; verdicts REPRODUCED-ROBUST / FRAGILE). Tests **18/18** (matrix+selection+sensitivity). |
| W1.7 Tests + env locks + gate | 🟢 done | Full suite **71 pass / 6 skip**; `tests/test_wp1_golden.py` **5/5** (artifact-gated, skips clean). `env/microfrb_repro.lock` refreshed (core vs harness) + `env/README.md`. **`docs/WP1_gate_memo.md`: GATE PASS → recommend Authorization B (WP2)**, §8.1 not triggered, FRB20211115A-fragile caveat carried forward. |

**✅ WP1 COMPLETE — gate PASS (reproduced-or-explained). Recommendation: proceed to WP2 / Authorization B.**
